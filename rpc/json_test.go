...
func TestJSONRequestParamsParsing(t *testing.T) {
	...
	for _, test := range validTests {
		...
		for i, arg := range args {
			...
			if arg.Kind() != expected.Kind() {
				t.Error("expected type for param %d in %s", i, test.input)
			}

			if arg.Kind() == reflect.Int && arg.Int() != expected.Int() {
				t.Error("expected int(%d), got int(%d) in %s", expected.Int(), arg.Int(), test.input)
			}

			if arg.Kind() == reflect.String && arg.String() != expected.String() {
				t.Error("expected string(%s), got string(%s) in %s", expected.String(), arg.String(), test.input)
			}
		}
	}
	...
	for i, test := range invalidTests {
		if _, err := codec.ParseRequestArguments(test.argTypes, test.input); err == nil {
			t.Error("expected test %d - %s to fail", i, test.input)
		}
	}
}
