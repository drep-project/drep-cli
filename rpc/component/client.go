...
func newClient(initctx context.Context, connectFunc func(context.Context) (net.Conn, error)) (*Client, error) {
	...
	c := &Client{
		...
		readResp:    make(chan []*rpcTypes.JsonrpcMessage),
...
func (c *Client) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	op := &requestOp{ids: []json.RawMessage{msg.ID}, resp: make(chan *rpcTypes.JsonrpcMessage, 1)}
...
func (c *Client) BatchCallContext(ctx context.Context, b []BatchElem) error {
	msgs := make([]*rpcTypes.JsonrpcMessage, len(b))
	op := &requestOp{
		ids:  make([]json.RawMessage, len(b)),
		resp: make(chan *rpcTypes.JsonrpcMessage, len(b)),
	}
	for i, elem := range b {
	...
	for n := 0; n < len(b) && err == nil; n++ {
		var resp *rpcTypes.JsonrpcMessage
...
func (c *Client) Subscribe(ctx context.Context, namespace string, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	...
	if c.isHTTP {
		return nil, rpcTypes.ErrNotificationsUnsupported
	}

	msg, err := c.newMessage(namespace+rpcTypes.SubscribeMethodSuffix, args...)
	if err != nil {
		return nil, err
	}
	op := &requestOp{
		ids:  []json.RawMessage{msg.ID},
		resp: make(chan *rpcTypes.JsonrpcMessage),
...
func (c *Client) newMessage(method string, paramsIn ...interface{}) (*rpcTypes.JsonrpcMessage, error) {
	params, err := json.Marshal(paramsIn)
	if err != nil {
		return nil, err
	}
	return &rpcTypes.JsonrpcMessage{Version: "2.0", ID: c.nextID(), Method: method, Params: params}, nil
}
...
func (c *Client) handleNotification(msg *rpcTypes.JsonrpcMessage) {
	if !strings.HasSuffix(msg.Method, rpcTypes.NotificationMethodSuffix) {
...
func (c *Client) handleResponse(msg *rpcTypes.JsonrpcMessage) {
...
func (c *Client) read(conn net.Conn) error {
	...
	readMessage := func() (rs []*rpcTypes.JsonrpcMessage, err error) {
		buf = buf[:0]
		if err = dec.Decode(&buf); err != nil {
			return nil, err
		}
		if rpcTypes.IsBatch(buf) {
			err = json.Unmarshal(buf, &rs)
		} else {
			rs = make([]*rpcTypes.JsonrpcMessage, 1)
